...
const SortableHeader: FC<{
  ...
  <TableHead className={className}>
    <Button variant="ghost" onClick={() => onClick(sortKey)} className="px-2">
      {label}
      <ArrowUpDown
        className={`ml-2 h-4 w-4 ${activeSortKey === sortKey ? '' : 'text-muted-foreground/50'}`}
      />
    </Button>
  </TableHead>
);
...
export function LeaderboardTable({ cases }: { cases: Case[] }) {
  const [sortKey, setSortKey] = useState<SortKey | null>('accusations');
        ...
        <Table>
          <TableHeader className="bg-muted/30">
            <TableRow>
              ...
              <SortableHeader label="Sospechoso" sortKey="mainSuspect" activeSortKey={sortKey} sortDirection={sortDirection} onClick={handleSort} />
              <TableHead className="hidden md:table-cell">Categoría</TableHead>
              <SortableHeader label="Acusaciones" sortKey="accusations" activeSortKey={sortKey} sortDirection={sortDirection} onClick={handleSort} className="text-right" />
              <SortableHeader label="Sentencias" sortKey="sentences" activeSortKey={sortKey} sortDirection={sortDirection} onClick={handleSort} className="text-right" />
